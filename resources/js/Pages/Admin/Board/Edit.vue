...
// --- 3. ส่วนจัดการ Font (เฉพาะ Editor) ---
...
const markToRemove = (imgUrl) => {
    if (!form.remove_images.includes(imgUrl)) form.remove_images.push(imgUrl);
            ...
            <div class="bg-white p-8 rounded-xl shadow-sm border border-slate-200">
                ...
                <form @submit.prevent="submit" class="space-y-6">
                    <div class="flex gap-4 items-start bg-slate-50 p-4 rounded-lg border border-slate-100">
                        ...
                        <div class="flex-grow">
                            <label class="block font-medium text-slate-700 mb-1">เปลี่ยนรูปปก</label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="col-span-2">
                            <label class="block font-medium text-slate-700 mb-1">หัวข้อ (Title)</label>
                            <input v-model="form.title" type="text" class="w-full border-slate-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 px-3 py-2" required />
                        </div>
                        <div class="col-span-2">
                            <label class="block font-medium text-slate-700 mb-1">คำโปรย (Subtitle)</label>
                            <input v-model="form.subtitle" type="text" class="w-full border-slate-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 px-3 py-2" />
                        </div>
                    </div>
                    ...
                    <div class="bg-slate-50 p-4 rounded-lg border border-slate-200 select-none">
                        <div class="flex justify-between items-center mb-3">
                            <div>
                                <label class="block font-medium text-slate-700">รูปภาพเพิ่มเติม (Gallery)</label>
                                <p class="text-xs text-slate-500">คลิกที่รูปเพื่อเลือกหลายรายการ</p>
                            </div>
                            <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 translate-y-1" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition ease-in duration-150" leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 translate-y-1">
                                <button v-if="selectedImages.length > 0" type="button" @click="deleteSelectedImages" class="text-sm text-white font-bold flex items-center gap-2 bg-red-500 hover:bg-red-600 px-4 py-2 rounded-full shadow-sm transition-colors">
                        ...
                        <div v-if="currentGallery.length > 0" class="grid grid-cols-4 md:grid-cols-6 gap-3 mb-4">
                            <div v-for="(img, idx) in currentGallery" :key="idx" class="relative group aspect-square rounded-lg overflow-hidden cursor-pointer border-2 transition-all duration-200" :class="{'border-red-500 shadow-md': isSelected(img), 'border-transparent hover:border-slate-300': !isSelected(img)}" @click="toggleSelection(img)">
                                ...
                                <button type="button" @click.stop="markToRemove(img)" class="absolute top-1 right-1 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 transition-opacity shadow-md hover:bg-red-600 hover:scale-110 z-10">
...
/* Apply sizes specifically to ck-content if wrapper class doesn't cascade */
